...
### (8) forward paper-trade（2026 OOS、backtest 未見データ）＋ 機構の分解
backtest は 2025-12-31 で打ち切り。執行可能 intraday を 2026 年データで forward に自動運転（`paper/`、PAPER ONLY、cron 07:30 JST）。
**データ衛生**: `1629.T`(Trading) に 2026-03-30/31 の yfinance 調整グリッチ（~0.57 vs 正常 ~288）があり、60日相関窓＝シグナルを汚染していた。despike（局所中央値の2倍乖離を除去）で修復（`paper/trader.py`）。下表・下図は修復後（backtest data/raw に同種グリッチ無し）。

| 指標 | 2026-01-28〜06-12（91営業日・真の OOS） |
|---|---|
| 勝率（gross>0） | 43% |
| 平均日次 gross | **-4.9 bp** |
| **gross（tradable intraday）** | 年率 **-12.3%**、Sharpe **-1.25** |
| net@2bp | 年率 -22.4%、Sharpe -2.27 |

**なぜ負か — 同じ建玉を gap と intraday に分解すると機構が見える：**

| 実現リターン | 累積(91日) | 勝率 | Sharpe |
|---|---|---|---|
| overnight GAP (co, 取れない) | **+18.1%** | 65% | **+6.55** |
| intraday DRIFT (oc, 取れる) | **-4.5%** | 43% | **-1.25** |

- **lead-lag は本物**：US の強弱は JP の寄りギャップ方向を強く当てる（GAP Sharpe +6.55、long バスケットは寄りで +18.9%）。**論文の中核仮説（日米セクター lead-lag が存在する）は forward でも真**。
- **だが寄りが過剰反応し、ザラ場で fade**：long バスケットは寄り +18.9% → ザラ場 **-7.8%**。**取れる intraday レッグは逆選択**（過剰に伸びた寄りを買って戻りに乗る）。
- 結論：**+18% の本物の alpha は丸ごと「取れないギャップ」に宿り、取れる残り(intraday)は過剰反応の反転で負ける**。§3「62%がギャップ」を forward が更に極端な形で再現。負の forward はデータ artifact ではなく実体（修復で gross -16%→-12% に縮小したが符号不変）。
- ※ この「寄りが過剰反応して fade」は **2026 限定**で一般則ではない（§9 で訂正）。派生 contrarian 仮説は §9 で pre-reg 検証 → **棄却**。

![forward](out/forward.png)

### (9) contrarian 仮説の pre-reg 検証 → 棄却（intraday の符号は安定でない）
§8 の 2026 の負から「ギャップを fade する contrarian intraday」仮説が出た。符号を 2026 で見て選ぶのは overfitting なので、仮説生成に使っていない **2019-2025 ホールドアウト＋年次**で pre-reg 検証（`paper/stability.py`、記録は `paper/HYPOTHESIS_contrarian.md`）：

| 年 | intraday oc(取れる) | gap co(取れない) |
|---|---|---|
| 2019 | +5.70 | +3.05 |
| 2020 | +1.51 | +1.87 |
| 2021 | +0.61 | +3.90 |
...
- **gap は8/8年プラス**（lead-lag は毎年・全レジームで本物）。**intraday は7/8年プラス**（2019-2025 は寄りが under-react で continuation＝momentum 方向が正）。**2026 だけ符号反転**（8年で1回）。
- ホールドアウト(2019-2025) で contrarian は gross Sharpe **-2.01**、net@2bp **-3.96** → 事前基準（net Sharpe>0.5）**FAIL**。
- **結論：contrarian は棄却**。2026 の fade は持続アノマリではなく単年のレジーム反転。取れる intraday の符号はむしろ momentum（7/8年）だが、既出のとおり honest cost で net≈0・流動性の壁(0/17)で死ぬ。**どちら向きにも運用不可**は不変、月単位 forward 検証に進む根拠なし。

![stability](out/stability.png)
